...
import edu.cmu.lti.oaqa.factory.EndToEndPipelineFactory;
...
public class OAQAFramework extends JCasAnnotator_ImplBase {
  ...
  public void initialize(UimaContext aContext) throws ResourceInitializationException {
    ...
    try {
      String configFilePath = ((String) aContext.getConfigParameterValue(PARAM_SYSTEM_CONFIG)).trim();
      String configText = FileUtils.file2String(new File(configFilePath));
      pipeline = EndToEndPipelineFactory.create( configText );
      LOGGER.info("\n------------------ Initialization ------------------");
      pipeline.initialize();
      MsgPrinter.enableErrorMsgs(true);
      int time = TimeKeeper.getInstance().getProcessTime("init",pipeline.getComponentId());
  ...
  @Override
    ...
    try {
      Question q = JCasManipulator.loadQuestion(ViewManager.getQuestionView(jcas));
      LOGGER.info("\n------------------ QID: "+q.getId()+" ------------------");
      LOGGER.info("Input question: \"" + q.getCoveredText() + "\"\n");

      pipeline.process(jcas);
      int time = TimeKeeper.getInstance().getProcessTime(q.getId(),pipeline.getComponentId());
