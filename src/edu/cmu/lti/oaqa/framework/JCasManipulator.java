...
public class JCasManipulator {
  ...
  private final static String PHRASE = "PHRASE";
	...
	public static List<AnswerCandidate> loadAnswerCandidates(JCas candidateView) throws CASException {
//	  if (!candidateView.getViewName().equals(ViewManager.CANDIDATE) && ! candidateView.getViewName().equals(ViewManager.FINAL_ANSWER)) {
...
//	  }
   ...
   * @param keyterms
   */
	public static void storeKeyTermsAndPhrases(JCas questionView, List<String> keyterms, List<String> keyphrases) {
	  storeConcepts(questionView, keyterms, keyphrases);
	}

	/**
	 * Store (overwrite) concepts (i.e. key terms / phrases in our implementation) into a view
	 * @param questionView
	 * @param keyterms
	 * @param type
	 */
	private static void storeConcepts(JCas questionView, List<String> keyterms, List<String> keyphrases) {
    // Remove old content first! (otherwise, it would work only once)
    Iterator<?> it = questionView.getJFSIndexRepository().getAllIndexedFS(AbstractQuery.type);
    while (it.hasNext()) {
      AbstractQuery query = (AbstractQuery) it.next();
    ...
    for (String keyterm : keyterms) {
      ...
      concepts = JCasManipulator.addToFSList(questionView, concepts, concept);
    }
    ...
    // Remove old content first! (otherwise, it would work only once)
	  Iterator<?> it = view.getJFSIndexRepository().getAllIndexedFS(AnswerList.type);
    while (it.hasNext()) {
      AnswerList answerCandidates = (AnswerList) it.next();
